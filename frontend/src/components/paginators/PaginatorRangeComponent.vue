...
const handleRangeChange = () => {
  const page =
    currentPage.value > totalPage.value
      ? Number.parseInt(totalPage.value)
      : Number.parseInt(currentPage.value)
  ...
  <div class="paginator-range | f-center g-10 mt-30">
    ...
    <div class="p-relative">
      ...
      <input
        ...
        :style="{
          background: `linear-gradient(to right, #582C4D 0%, #582C4D ${progress - 0.4}%, #e2e2e2 ${progress - 0.4}%, #e2e2e2 100%)`,
        }"
      />
    </div>
    <button
...
.paginator-range {
  ...
  &_input {
    ...
    &::-webkit-slider-thumb {
      ...
      width: 16px;
      height: 16px;
      background: var(--primary); /* Couleur du thumb */
