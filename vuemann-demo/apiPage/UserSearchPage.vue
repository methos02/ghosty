...
const mockUsers = [
  ...
  },
  {
    id: 4,
    username: 'marann',
    fullname: 'MARTIN Anne',
  },
  {
    id: 5,
    username: 'marpie',
    fullname: 'MARTIN Pierre',
  },
  {
    id: 6,
    username: 'marcla',
...
const mockUserSearch = async (search, options) => {
  ...
  const field = typeSearch === 'username' ? 'username' : 'fullname'
  const searchFormat = search.toLowerCase()

  const filteredUsers = mockUsers.filter(user => user[field].toLowerCase().includes(searchFormat))
...
<template>
  ...
  <p class="my-10">
    ...
    Il offre la possibilité de rechercher par nom complet ou nom d'utilisateur, avec pagination
    infinie et affichage dans un dropdown. Ce composant fait partie de l'API Users de Vuemann.
  </p>
  ...
  <table class="t-default">
    ...
    <tbody>
      ...
      <tr>
        ...
        <td>undefined</td>
        <td>
          Fonction de callback personnalisée pour la recherche. Si fournie, elle remplace les appels
          API par défaut.
        </td>
      </tr>
    </tbody>
  ...
  <h2 class="h2">v-model & événements</h2>
  <table class="t-default">
    <thead>
      <tr>
        <th>Nom</th>
    ...
    <tbody>
      <tr>
        <td>v-model:open</td>
        <td>Boolean</td>
        <td>Ouvre ou ferme le dropdown des résultats (déclaratif, transmis à InputSearch).</td>
      </tr>
      <tr>
        <td>@select</td>
        <td>user</td>
        <td>
          Émis lorsqu'un utilisateur est sélectionné via
          <span class="badge-code">select(user)</span> du slot.
        </td>
      </tr>
    </tbody>
  </table>
  ...
  <table class="t-default">
    ...
    <tbody>
      <tr>
        <td>focus</td>
        <td>-</td>
        <td>Donne le focus au champ de recherche.</td>
      </tr>
    </tbody>
  </table>
  ...
  <p>
    ...
    Dans cette page de démonstration, les données utilisateurs sont simulées. Le composant utilise
    un callback personnalisé pour éviter d'avoir besoin des routes API protégées. Nom de famille
    disponible: Martin, Dubois.
  </p>
  ...
  <CodeHtml>
    <scriptBalise v-pre>
      import UserSearch from '@brugmann/vuemann/src/apis/users/views/UserSearchComponent.vue';
      import { ref } from 'vue'; const selectedUsers = ref([]);
    </scriptBalise>
    <template v-pre>
