<template>
  ...
  <p>
    Le système de formulaires Vuemann fournit un ensemble complet de tests de validation prêts à
    l'emploi. Ces tests permettent de valider automatiquement les données des formulaires selon des
    règles métier définies.
  </p>

  <h2 class="h2">Liste Complète des Tests</h2>
  <p>Vuemann fournit 19 tests de validation :</p>
  <ul class="list ml-25">
    <li><strong>Tests de base</strong> : required, in</li>
    <li><strong>Tests de format</strong> : email</li>
    <li><strong>Tests numériques</strong> : integer, positive, min, max</li>
    <li>
      <strong>Tests de dates</strong> : date, datePast, dateFutur, dateStrictPast, dateStrictFutur
    </li>
    <li><strong>Tests de taille</strong> : size, sizeMin, sizeMax</li>
    <li><strong>Tests belges</strong> : niss, biss, inami, passport</li>
  </ul>
  ...
  <p>
    Valide qu'un champ est obligatoire. Ce test gère les chaînes de caractères, tableaux, objets,
    booléens et nombres.
  </p>
  <div class="div-code">
    <span class="color-neutral-500">// Validation - Valeurs invalides</span><br />
    const result = tests.required('') // Retourne 'field_required'<br />
    const result = tests.required(null) // Retourne 'field_required'<br />
    const result = tests.required(undefined) // Retourne 'field_required'<br />
    const result = tests.required([]) // Retourne 'field_required' (tableau vide)<br />
    const result = tests.required({}) // Retourne 'field_required' (objet vide)<br />
    <br />
    <span class="color-neutral-500">// Validation - Valeurs valides</span><br />
    const result = tests.required('valeur') // Retourne '' (valide)<br />
    const result = tests.required([1, 2, 3]) // Retourne '' (tableau non-vide)<br />
    const result = tests.required({key: 'val'}) // Retourne '' (objet non-vide)<br />
    const result = tests.required(0) // Retourne '' (0 est valide)<br />
    const result = tests.required(false) // Retourne '' (false est valide)
  </div>
  <p><strong>Usage dans un formulaire :</strong></p>
  <div class="div-code">
    'username': 'required'<br />
    'email': 'required|email'<br />
    'services': 'required' <span class="color-neutral-500">// Pour un tableau de services</span
    ><br />
    'config': 'required' <span class="color-neutral-500">// Pour un objet de configuration</span>
  </div>
  ...
  <h2 class="h2">Tests de Format</h2>

  <h3 class="h3">`email`</h3>
  <p>Valide qu'une valeur est une adresse email valide selon le format RFC 5322.</p>
  <div class="div-code">
    <span class="color-neutral-500">// Validation</span><br />
    const result = tests.email('user@example.com') // Retourne '' (valide)<br />
    const result = tests.email('user@localhost') // Retourne '' (valide)<br />
    const result = tests.email('invalid.email') // Retourne 'field_email'<br />
    const result = tests.email('user@') // Retourne 'field_email'<br />
    const result = tests.email('@example.com') // Retourne 'field_email'
  </div>
  <p><strong>Usage dans un formulaire :</strong></p>
  <div class="div-code">
    'email': 'required|email'<br />
  ...
  <p>Valide qu'une date est dans le futur (aujourd'hui inclus).</p>
  <div class="div-code">
    <span class="color-neutral-500">// Validation</span><br />
    const options = 'DD/MM/YYYY'<br />
    const result = tests.dateFutur('17/12/2025', options) // Retourne '' (valide)<br />
    const result = tests.dateFutur(today, options) // Retourne '' (aujourd'hui inclus)<br />
    const result = tests.dateFutur('15/01/2020', options) // Retourne 'date_not_futur'
  </div>
  <p><strong>Usage dans un formulaire :</strong></p>
  <div class="div-code">
  ...
  <h3 class="h3">`dateStrictPast`</h3>
  <p>Valide qu'une date est strictement dans le passé (aujourd'hui exclus).</p>
  <div class="div-code">
    <span class="color-neutral-500">// Validation</span><br />
    const options = 'DD/MM/YYYY'<br />
    const result = tests.dateStrictPast('15/01/2020', options) // Retourne '' (valide)<br />
    const result = tests.dateStrictPast(today, options) // Retourne 'date_not_past' (aujourd'hui
    exclus)<br />
    const result = tests.dateStrictPast('17/12/2025', options) // Retourne 'date_not_past'
  </div>
  <p><strong>Usage dans un formulaire :</strong></p>
  <div class="div-code">
    'completionDate': 'dateStrictPast:DD/MM/YYYY'<br />
    'birthDate': 'dateStrictPast:YYYY-MM-DD'
  </div>
  <p>
    <strong>Différence avec datePast :</strong> <span class="badge-code">datePast</span> accepte
    aujourd'hui, <span class="badge-code">dateStrictPast</span> l'exclut.
  </p>

  <h3 class="h3">`dateStrictFutur`</h3>
  <p>Valide qu'une date est strictement dans le futur (aujourd'hui exclus).</p>
  <div class="div-code">
    <span class="color-neutral-500">// Validation</span><br />
    const options = 'DD/MM/YYYY'<br />
    const result = tests.dateStrictFutur('17/12/2025', options) // Retourne '' (valide)<br />
    const result = tests.dateStrictFutur(today, options) // Retourne 'date_not_futur' (aujourd'hui
    exclus)<br />
    const result = tests.dateStrictFutur('15/01/2020', options) // Retourne 'date_not_futur'
  </div>
  <p><strong>Usage dans un formulaire :</strong></p>
  <div class="div-code">
    'appointmentDate': 'dateStrictFutur:DD/MM/YYYY'<br />
    'deliveryDate': 'dateStrictFutur:YYYY-MM-DD'
  </div>
  <p>
    <strong>Différence avec dateFutur :</strong> <span class="badge-code">dateFutur</span> accepte
    aujourd'hui, <span class="badge-code">dateStrictFutur</span> l'exclut.
  </p>
  ...
  <p><strong>Usage dans un formulaire :</strong></p>
  <div class="div-code">'socialSecurityNumber': 'niss'</div>
  ...
  <p><strong>Usage dans un formulaire :</strong></p>
  <div class="div-code">'nationalNumber': 'biss'</div>

  <h3 class="h3">`inami`</h3>
  <p>
    Valide un numéro INAMI/RIZIV belge (11 chiffres : 6 base + 2 contrôle + 3 qualification). Le
    chiffre de contrôle est validé avec l'algorithme modulo 97 (avant 2019) ou modulo 89 (après
    2019). Les séparateurs (tirets, points, espaces) sont automatiquement retirés.
  </p>
  <div class="div-code">
    <span class="color-neutral-500">// Validation</span><br />
    const result = tests.inami('20000014001') // Retourne '' (valide, mod97)<br />
    const result = tests.inami('20000072001') // Retourne '' (valide, mod89)<br />
    const result = tests.inami('2-00000-14-001') // Retourne '' (valide, avec séparateurs)<br />
    const result = tests.inami('20000099001') // Retourne 'inami_invalid' (clé de contrôle
    incorrecte)<br />
    const result = tests.inami('ABCDEFGHIJK') // Retourne 'inami_invalid' (non numérique)
  </div>
  <p><strong>Usage dans un formulaire :</strong></p>
  <div class="div-code">
    'inamiNumber': 'inami'<br />
    'doctorNumber': 'required|inami'
  </div>

  <h3 class="h3">`passport`</h3>
  <p>
    Valide un numéro de passeport belge (2 lettres majuscules suivies de 7 chiffres). Les espaces
    sont automatiquement retirés.
  </p>
  <div class="div-code">
    <span class="color-neutral-500">// Validation</span><br />
    const result = tests.passport('EJ1234567') // Retourne '' (valide)<br />
    const result = tests.passport('ej1234567') // Retourne 'passport_invalid' (minuscules)<br />
    const result = tests.passport('EJK123456') // Retourne 'passport_invalid' (format incorrect)
  </div>
  <p><strong>Usage dans un formulaire :</strong></p>
  <div class="div-code">
    'passportNumber': 'passport'<br />
    'travelDocument': 'required|passport'
  </div>

  <h2 class="h2">Combinaison de Règles</h2>
  <p>Les tests peuvent être combinés avec le séparateur <span class="badge-code">|</span> :</p>
  <div class="div-code">
    const formRules = {<br />
  ...
  <ul class="list ml-25">
    ...
    <li><strong>Email invalide</strong> : <span class="badge-code">field_email</span></li>
    <li>
      <strong>Valeur hors limites</strong> :
      <span class="badge-code">field_min:min=[valeur]</span> ou
      <span class="badge-code">field_max:max=[valeur]</span>
    </li>
    <li>
      <strong>Taille incorrecte</strong> :
      <span class="badge-code">field_size_equal:size=[valeur]</span>,
      <span class="badge-code">field_size_min:size=[valeur]</span>,
      <span class="badge-code">field_size_max:size=[valeur]</span>
    </li>
    <li>
      <strong>Valeur non autorisée</strong> : <span class="badge-code">field_in:in=[options]</span>
    </li>
    <li><strong>Date invalide</strong> : <span class="badge-code">date_invalid</span></li>
    <li><strong>Date pas dans le passé</strong> : <span class="badge-code">date_not_past</span></li>
    <li>
      <strong>Date pas dans le futur</strong> : <span class="badge-code">date_not_futur</span>
    </li>
    <li>
      <strong>NISS invalide</strong> : <span class="badge-code">niss_invalid</span>,
      <span class="badge-code">niss_date</span>
    </li>
    <li>
      <strong>BISS invalide</strong> : <span class="badge-code">biss_invalid</span>,
      <span class="badge-code">biss_date</span>
    </li>
    <li><strong>INAMI invalide</strong> : <span class="badge-code">inami_invalid</span></li>
    <li><strong>Passeport invalide</strong> : <span class="badge-code">passport_invalid</span></li>
  </ul>
  ...
  <p>
    Le système de tests par défaut de Vuemann fournit une base solide et extensible pour la
    validation des formulaires. En combinant ces tests avec des règles métier personnalisées, vous
    pouvez créer des formulaires robustes et maintenables.
  </p>
</template>
