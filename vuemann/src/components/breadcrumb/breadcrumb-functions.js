...
const init = route => {
  ...
  for (const routeMatched of route.matched) {
    if (routeMatched.meta.breadcrumb === undefined) {
      return links
    }
    ...
    if (route.meta.breadcrumb.parents !== undefined) {
      return breadcrumb.resolveParents(route.meta.breadcrumb.parents, links)
    }

    links.push({
      ...routeMatched.meta.breadcrumb,
      route: { name: routeMatched.meta.breadcrumb.route },
    })
  }
...
const resolveParents = (parents, links) => {
  if (!Array.isArray(parents) || parents.length === 0) {
    return flash.errorT('breadcrumb.errors.parents')
  }

  for (const parent of parents) {
    const route = servicesM.service('router:getRoute', parent.route)
    if (route === undefined) {
      return flash.errorT('breadcrumb.errors.route_unknown', { route_name: parent.route })
    }
