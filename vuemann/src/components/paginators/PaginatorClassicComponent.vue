...
const props = defineProps({
  ...
  loadingPage: { type: Number, default: undefined },
})
...
const generateLinks = () => {
  ...
  for (let page_number = 1; page_number <= totalPage.value; page_number++) {
    if (page_number === 3 && currentPage.value >= 6) {
      links.push('...')
    }

    if (pagesLinked().includes(page_number)) {
      links.push({ page_number })
    }

    if (page_number === totalPage.value - 3 && currentPage.value <= totalPage.value - 5) {
      links.push('...')
    }
  }
    ...
    <ul class="paginator | d-flex a-center py-15">
      ...
      <template v-if="currentPage === 1">
        <li
          ...
          aria-disabled="true"
          :aria-label="t('paginator.previous_page')"
          data-button="previous"
        >
          <span
            class="paginator-chevron"
            aria-hidden="true"
          >
      ...
      <template v-else>
        <li class="paginator-li">
          <button
            ...
            :disabled="loadingPage !== undefined"
            rel="prev"
            :aria-label="t('paginator.previous_page')"
            data-button="previous"
          >
      ...
      >
        ...
        <template v-else-if="typeof link === 'object' && loadingPage === link.page_number">
          <li
            class="paginator-li paginator-loader"
        ...
        <template v-else-if="typeof link === 'object'">
          ...
          >
            <button
              type="button"
              @click="handlePageClick(link.page_number)"
              class="paginator-button"
              :disabled="loadingPage !== undefined"
              :data-page="link.page_number"
            >
              {{ link.page_number }}
            </button>
          </li>
      ...
      <template v-if="currentPage !== totalPage">
        <li class="paginator-li">
          <button
            ...
            :disabled="loadingPage !== undefined"
            rel="next"
            :aria-label="t('paginator.next_page')"
            @click="handlePageClick(currentPage + 1)"
            data-button="next"
          >
...
.paginator-container {
  .paginator {
    ...
    &-button {
      ...
      &:hover {
        background-color: color.adjust(variables.$neutral-800, $lightness: 40%, $space: hsl);
      }
