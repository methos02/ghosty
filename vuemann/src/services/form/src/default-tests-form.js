...
import { inamiTests } from '@brugmann/vuemann/src/services/form/src/defaultTests/inami-test.js'
import { FormHelper } from '@brugmann/vuemann/src/helpers/form-helper.js'

export const tests = {
  required: value => (FormHelper.isEmpty(value) ? 'field_required' : ''),
  in: (value, options) =>
    options.options.split(',').includes(value) ? '' : 'field_in:in=' + options.options,
  ...
  inami: inamiTests.inami,
  passport: belgianNumberTests.passport,
  integer: value => (/^\d+$/.test(value) ? '' : 'field_invalid'),
  positive: value => (/^\d+$/.test(value) && Number.parseInt(value) >= 0 ? '' : 'field_invalid'),
  min: (value, options) =>
    Number.parseInt(value) < Number.parseInt(options.options)
      ? 'field_min:min=' + options.options
      : '',
  max: (value, options) =>
    Number.parseInt(value) > Number.parseInt(options.options)
      ? 'field_max:max=' + options.options
      : '',
  size: (value, options) =>
    value.length === Number.parseInt(options.options)
      ? ''
      : 'field_size_equal:size=' + options.options,
  sizeMin: (value, options) =>
    value.length < options.options ? 'field_size_min:size=' + options.options : '',
  sizeMax: (value, options) =>
    value.length > options.options ? 'field_size_max:size=' + options.options : '',
  email: value =>
...
const execute = (test, value, datas) => {
  ...
  if (tests[test_name] === undefined) {
    return flash.error(`Le test "${test_name}" est inconnu.`)
  }
