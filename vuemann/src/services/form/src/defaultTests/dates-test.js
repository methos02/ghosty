...
const datePast = (dateString, format) => {
  ...
  return dateToTest.getTime() > today ? 'date_not_past' : ''
}
...
const dateFutur = (dateString, format) => {
  ...
  return dateToTest.getTime() < today ? 'date_not_futur' : ''
}
...
export const createDate = (dateString, format) => {
  ...
  return !(date instanceof Date) || date.getMonth() !== month - 1 || date.getDate() !== day
    ? 'date_invalid'
    : date
}
...
const defineDateComposents = (dateMatch, format) => {
  ...
  return [order.year, order.month, order.day]
    .map((pos, index) => ({ pos, value: Number(dateMatch[index + 1]) }))
