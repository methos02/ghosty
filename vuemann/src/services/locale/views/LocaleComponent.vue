...
const locales = ConfigLoader.find('locales', {})
const dropdown = ref()
const { locale } = useI18n({ useScope: 'global' })

const changeLocale = async new_locale => {
  localeStore.set(new_locale)
    ...
    <template v-slot:items>
      <ul class="f-column g-5">
        ...
        >
          ...
          >
            <img
              :src="`/images/vuemann/${initial}.png`"
              :alt="t('locale.lang_change', { lang })"
            />
            <span>{{ lang }}</span>
          </button>
        </li>
